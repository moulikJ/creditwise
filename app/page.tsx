...
export default function HomePage() {
  ...
  return (
    <Shell>
      <section className="grid items-center gap-8 py-7 md:grid-cols-[1.05fr_.95fr] md:py-14">
        <ScrollReveal>
          ...
          <h1 className="mt-4 max-w-2xl text-4xl font-semibold leading-tight md:text-5xl" style={{ fontFamily: "var(--font-display)" }}>
            Find the card that pays you the most.
          </h1>
          <p className="mt-4 max-w-xl text-fg-2 md:text-lg">
          ...
          <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted">
            <span>✓ Reward maths included</span><span>✓ Fees accounted for</span><span>✓ Official-source checks</span>
          </div>
        </ScrollReveal>
          ...
          <div className="relative mx-auto h-80 max-w-xl">
            {featured.map((c, i) => (
              <div key={c.id} className="absolute" style={{ left: `${i * 70}px`, top: `${i * 34}px`, zIndex: i }}>
                <div className="transition-all duration-500 hover:-translate-y-4 hover:scale-105" style={{ transform: `rotate(${i * 3 - 4}deg)` }}><CardArt card={c} className="h-44 w-72" /></div>
              </div>
            ))}
            <div className="absolute bottom-0 right-0 rounded-xl border border-line bg-surface/95 p-4 shadow-lg backdrop-blur">
